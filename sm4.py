...
bot = telebot.TeleBot('7321056264:AAHkFmaXhOaVAbpw1cCsyiI0fRsoK_n7udA')
...
BOT_LINK = "@miracle4_bot"
miracle4_bot_link = BOT_LINK.replace('_', '\\_')
...
def create_random_key(length=15):
    ...
    custom_key = f"VIP-RAHUL-{random_key}"
    return custom_key
...
def start_command(message):
    ...
    bot.reply_to(message, "𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 *PRIVATE DDOS™* 𝗯𝗼𝘁 !", reply_markup=markup, parse_mode='Markdown')
    bot.send_message(
        message.chat.id,
...
def process_attack_details(message):
    ...
    if len(details) == 3:
        ...
        try:
            ...
            time = int(details[2])
            if time > 5000:
                response = "❗️𝗘𝗿𝗿𝗼𝗿 : 𝘂𝘀𝗲 𝗹𝗲𝘀𝘀 𝘁𝗵𝗲𝗻 5000  𝘀𝗲𝗰𝗼𝗻𝗱𝘀❗️"
            else:
                # Record and log the attack
                record_command_logs(user_id, 'attack', target, port, time)
                log_command(user_id, target, port, time)
                full_command = f"./SOULCRACK <IP> <PORT> <DURATION>"
...
def my_info(message):
    ...
    response = (
    f"👤 𝗨𝗦𝗘𝗥 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗧𝗜𝗢𝗡 👤\n\n"
    f"ℹ️ 𝗨𝘀𝗲𝗿𝗻𝗮𝗺𝗲 : @{miracle4_username}\n"
    f"🆔 𝗨𝘀𝗲𝗿𝗜𝗗 : {user_id}\n"
    f"🚹 𝗥𝗼𝗹𝗲 : {role}\n"
    f"🕘 𝗘𝘅𝗽𝗶𝗿𝗮𝘁𝗶𝗼𝗻 : {miracle4_key_expiration}\n"
    )   
